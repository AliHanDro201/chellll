...
prompt = """
            ...
            - Функция process_user_input проверяет, начинается ли введённый текст с триггера "обратись к gpt","обратись в gpt" и похожие запросы с обращением к gpt. Если да – вызывается функция orchestrate_browser_chat, которая делает запрос через браузер. Когда пользователь говорит Обратиться в GPT". ******" поиск в интернете ты не должен делать. Он просит тебя обратиться к GPT в барузере. Определяй когда это нужно искать в интернете допустим если скажут "найди мне информацию о..." или "поискать в интернете..." и т.д.
            Любое похожее слово по типу: обратись, обратится и тд. если там есть ещё слово GPT или gpt то это значит что нужно вызвать функцию orchestrate_browser_chat. 
