...
def transcribe_audio(b64_audio: str) -> str:
    ...
    # —— опциональная фильтрация по группе языков ——
    
    has_cyr = bool(re.search(r"[А-Яа-я]", text))
    has_kaz = bool(re.search(r"[әӘөӨұҰқҚғҒңҢһҺіІ]", text))
    has_lat = bool(re.search(r"[A-Za-z]", text))
    if not (has_cyr or has_kaz or has_lat):
        # если не обнаружили ни одного из трёх алфавитов,
